...
const char vers[]="\0$VER: PolyglotMan 3.3-aguide (05.02.2021)\0";
typedef enum {false, true} bool;
bool astart; // AmigaGuide conversion can start
...
const char PROVENANCE[] =
	"manual page generated by PolyglotMan v" POLYGLOTMANVERSION;
const char HOME[] = "from http://polyglotman.sourceforge.net";
...
guide(enum command cmd) {
	...
	switch (cmd) {
	   ...
	   case CHARCENT:
		...
		case CHARAT:
		 printf("\\@");
		 break; // AmigaOS 4 version has this break
	   case CHARTAB:
		printf("@{tab}");
	   default:
		break;
	...
	switch (cmd) {
	   case ENDDOC:
		...
		if (!tocc) {
			...
			printf("@{fg shine}@{b}%s@{ub}@{fg text}\n\n", TABLEOFCONTENTS);
			for (i=0, lasttoc=BEGINSECTION; i<tocc; lasttoc=toc[i].type, i++) {
				if (lasttoc!=toc[i].type) {
       ...
       /* BUG: comments are translated into spaces */
	   case COMMENTLINE:
	     if (astart == true)
	       printf("\n@remark");
		 break;
...
*/
		   ...
		   case 'v': /*case '?':*/
			printf("PolyglotMan v" POLYGLOTMANVERSION " of $Date: 05-02-2021 15:54:48 $\n");
			exit(0);
